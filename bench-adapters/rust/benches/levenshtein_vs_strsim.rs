//! Head-to-head: StringCheese's Levenshtein vs. `strsim` 0.11.
//!
//! # Representation caveat
//!
//! `strsim::levenshtein` operates on `&str` and iterates `chars`
//! internally; StringCheese's `Levenshtein.distance` operates on `&[T:
...
//! groups:
//!
//! * `levenshtein/vs_strsim_str_ascii` — realistic usage. StringCheese on
//!   bytes, strsim on `&str`. This is the "if you swap `strsim` for
//!   StringCheese in your app" number, and it deliberately gives
//!   StringCheese the fast-path advantage.
//! * `levenshtein/vs_strsim_generic_bytes` — algorithm-only. StringCheese
...
//! identical}) × (implementation ∈ {stringcheese, strsim}). Matches
//! `stringcheese-bench`'s Levenshtein sweep.
