# Comparand Rust Bench Adapters

Head-to-head criterion benches comparing Comparand's core algorithms
against the Rust ecosystem's two most-used sequence-comparison crates.

This crate is deliberately **not** a member of the outer Comparand
...
implementation**. Lengths and regimes match `comparand-bench` exactly
(`LENGTHS = [8, 32, 128, 512, 2048]`, regimes = `random`, `similar`,
`identical`) so a criterion report from this crate and a criterion
report from `comparand-bench` line up on the same axis.

| Comparand algorithm | `strsim`                        | `rapidfuzz`                    | Bench file                           |
|---------------------|---------------------------------|--------------------------------|--------------------------------------|
...
Comparand's algorithm crates consume `&[T: Eq]` — most usefully,
...
* **`levenshtein_vs_strsim.rs`** carries two groups.
  * `vs_strsim_str_ascii` — Comparand on `&[u8]` vs. `strsim` on `&str`.
    This is the realistic-usage comparison and deliberately gives
    Comparand the byte-slice fast-path advantage.
  * `vs_strsim_generic_bytes` — Comparand on `&[u8]` vs.
...
* **`jaro_vs_strsim.rs`** / **`hamming_vs_strsim.rs`** /
  **`damerau_vs_strsim.rs`** compare Comparand-on-bytes to
...
| Common name              | Comparand                    | `strsim`                | `rapidfuzz`                          |
|--------------------------|------------------------------|-------------------------|--------------------------------------|
...
which matches Comparand's `Damerau`; pairing it with Comparand's
...
* Comparand's banded Levenshtein at `k = 3` (the classical
...
* `strsim` has no bounded variant of any of its algorithms. The
  Comparand cutoff kernels are still benched in the `_vs_strsim.rs`
  files (e.g. `hamming/within_k3_comparand_only`) as
  Comparand-only groups so that a reader can read across from the
  strsim head-to-head to the Comparand cutoff-vs-Comparand
...
`comparand-bench::inputs` (see `src/lib.rs::Rng` for the source note).
...
is to see how a Comparand kernel and its ecosystem alternative scale
