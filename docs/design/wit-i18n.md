...
### 8.7. Phase 5 follow-up — UAX #14 line breaking (2026-08)

**Landed.** Closes the "UAX #14 line breaking deferred to a separate crate" note from Phase 5 (§ 8.5 above). Ships across a single wave the algorithm crate, WIT interface, WASM component wrapper, SCUD extension, CI jobs, and doc entry:

- `stringcheese-scud` — extended with the `CAP_LINEBREAK` capability tag (`"LBRK"`) plus three new section ids (`SECT_LB_CLASSES` / `SECT_LB_RULES` / `SECT_LB_TAILORINGS`), a 43-value `LineBreakClass` enum covering the full UAX #14 class set (OP / CL / CP / QU / GL / NS / EX / SY / IS / PR / PO / NU / AL / HL / ID / IN / HY / BA / BB / B2 / ZW / CM / WJ / H2 / H3 / JL / JV / JT / RI / EB / EM / ZWJ / CJ / SG / AI / CB / BK / CR / LF / NL / SP / SA + XX default), a `LineBreakDataView` zero-copy view, and a `LineBreakSectionBuilder` writer. Tailoring section carries a single `u8` CJK strictness byte per UAX #14 § 6.1 (`LB_STRICTNESS_LOOSE` / `LB_STRICTNESS_NORMAL` / `LB_STRICTNESS_STRICT`). `RULES_UAX14_DEFAULT` marker + `set_default_rules()` builder helper wire the "just use the algorithm's default rules" path. `ScudFile::as_linebreak_data()` projects the body view. +4 new SCUD unit tests (round-trip, empty-pack, class-enum round-trip, capability-mismatch guard).
- `stringcheese-icu-linebreak` — the WIT interface at `component/wit/linebreak/stringcheese-icu-linebreak.wit` (package `tegmentum:i18n-linebreak@0.1.0`, parses cleanly under `wit-parser` with 3 smoke tests) plus a `LineBreakEngine` algorithm side that consumes an optional `LineBreakPack` and drives the full UAX #14 rule set (LB1 class resolution through LB31 default any-allow, with LB30a / LB30b) from the built-in classification table in `classes.rs`. Pair-table implementation is a direct rule-by-rule translation, not a compressed table — each numbered rule maps to a named branch in `pair_decision` for readability, and reviewer cross-referencing to the spec goes rule-number → branch-name in one hop. `find_breaks(text)` and `find_breaks_with_strictness(text, strictness)` both emit `Vec<BreakOpportunity>` where each opportunity carries `{offset: u32, kind: BreakKind::Mandatory | ::Allowed}`. Strictness gets stored on the engine so `LineBreakEngine::new().with_strictness(Strictness::Strict)` composes cleanly. **53 unit tests** covering every numbered rule LB2-LB31 (grouped by rule id), plus **111 hand-picked LineBreakTest.txt vectors** in `tests/uax14_vectors.rs` running under "superset" mode — the algorithm must emit at least every enumerated break, extra breaks are tolerated so future classification-table growth doesn't silently regress. Pass rate: 111/111 (100%) on the shipped subset.
- `stringcheese-icu-linebreak-component` — the WASM component wrapper that carries the `linebreak-world` world across the wasm boundary, mirroring the `stringcheese-icu-segment-component` template's shape (dual `cdylib` + `rlib`, `wit-component` feature gate on `wit-bindgen-rt` 0.44, pre-generated `src/bindings.rs` from `wit-bindgen rust --runtime-path wit_bindgen_rt`, in-process `wasmtime::component::bindgen!` smoke tests, `AtomicPtr`-backed `shared_engine()` singleton). Like the segment component, the linebreak component embeds **no** SCUD packs — the reference engine is just `LineBreakEngine::new()`. `supported-locales` returns `[""]` (the root-locale marker); `supports(loc)` returns `true` for every input for forward compatibility. **10 wasmtime smoke tests** cover hard breaks (CRLF, LF, NEL), soft breaks after space and after hyphen, no-break within numeric run, empty input, all three strictness variants, and both capabilities exports.
- CI — two new jobs `wasm-i18n-linebreak` (native unit tests + wit-parser gate) and `wasm-i18n-linebreak-component` (wasm32-wasip1 build + componentise + wasmtime smoke) in `.github/workflows/ci.yml`. The linebreak-component crate is added to the wasm-runtime job's `--exclude` list for the same duplicate-export-symbol reason the other icu-*-component crates are excluded.

**Wasm component sizes** (measured locally, aarch64 macOS, `--release`, wasmtime 26 / wasm-tools 1.219): raw module 56 739 B, componentised 79 193 B. Smaller than the segment component (~82 KiB raw / ~105 KiB componentised) — the LB rule set is larger in number of rules but each rule is a straightforward pair-classification check rather than the multi-state fold that UAX #29's WB4 / SB5 need, so the compiled `pair_decision` boils down to a single function of moderate size.

**SCUD `LineBreakClass` table size.** The 43-value enum + `from_u8` / `as_u8` accessors add ~450 lines of source in `stringcheese-scud/src/lib.rs`. The section-builder + view + parse helpers add ~130 more lines. Every enum variant is a wire-stable `#[repr(u8)]` discriminant; `from_u8` maps unknown bytes to `Xx` (the "unknown → AL under LB1" invariant).

**Algorithm crate LOC.** `stringcheese-icu-linebreak/src/lib.rs` — 1030 lines including doc comments + 53 unit tests. `stringcheese-icu-linebreak/src/classes.rs` — 380 lines for the built-in classifier. `tests/uax14_vectors.rs` — 440 lines for the 111-vector conformance subset. `tests/wit_parse.rs` — 65 lines.

**Rules fully vs approximated.** Full: LB1 (class resolution), LB2 (sot), LB3 (eot), LB4 (BK mandatory), LB5 (CR × LF / CR/LF/NL mandatory), LB6 (no break before hard terminator), LB7 (no break before SP / ZW), LB8 (break after ZW), LB8a (ZWJ ×), LB9 (CM/ZWJ fold into preceding class, except after mandatory-break classes / SP / ZW), LB11 (WJ), LB12 / LB12a (GL glue), LB13 (no break before CL/CP/EX/IS/SY), LB14 (OP × any across SPs), LB15 (QU × OP), LB16 ((CL|CP) × NS), LB17 (B2 × B2), LB18 (break after SP), LB19 (× QU / QU ×), LB20 (CB either side allows break), LB21 (× BA / × HY / × NS / BB ×), LB21a / LB21b (Hebrew-letter hyphen glue), LB22 (× IN), LB23 / LB23a (alpha ↔ num, PR ↔ ID/EB/EM), LB24 (PR/PO ↔ AL/HL), LB25 (numeric expressions), LB26 / LB27 (Hangul syllable composition), LB28 (AL/HL glue), LB29 (IS × AL), LB30 (AL/HL/NU × OP, CP × AL/HL/NU), LB30a (RI-pair parity), LB30b (EB × EM), LB31 (default any-allow).

Approximated / partial: LB1's `SA → AL` fold is a Phase 5 approximation — the real per-script behaviour (Thai / Lao / Khmer / Burmese dictionary-based line breaking) is a documented deferral below. LB1's `AI → AL` fold matches the CLDR default; the East-Asian-Width tailoring (`AI → ID` when the ambient font is EA-W-wide) is not implemented — see the LB30 note below.

**Red flags.** A couple worth noting for reviewers:

- **LB25 numeric-expression pair table is simplified.** The full UAX #14 LB25 has a per-side table listing every allowed transition inside a numeric run (PR × OP, PR × NU, PO × OP, PO × NU, OP × NU, HY × NU, IS × NU, NU × NU/SY/IS, NU (SY|IS)* × NU, NU (SY|IS|CL|CP)* × PO/PR). The shipped implementation collapses this into a stateful `in_numeric` flag that enters on `NU` and re-arms on `SY | IS | CL | CP | PO | PR | HY | BA`, plus per-position checks for the entry / exit edges (`(PR|PO) × NU/OP`, `(OP|HY|IS) × NU`, `NU × NU/SY/IS/CL/CP/PO/PR`). Cross-checked against the shipped `$1,234.56`, `100%`, `1/2`, `1,234`, `£1,000`, `€2.50`, `¥100` vectors. A pathological input like `(1)PR NU CL` might diverge from the CLDR reference on a specific interior break; a future round can grow `pair_decision` into a proper LB25 sub-table if that ever surfaces.
- **LB30 East-Asian-Width interaction not implemented.** UAX #14 § 6 LB30 says "do not break between letters (AL | HL | NU) and OP" but *excluding full-width variants of OP* (which need their own break) — those exclusions come from the East-Asian-Width property, which we don't ship. The impact: for the small set of full-width open-punctuation code points (e.g. U+FF08 FULLWIDTH LEFT PARENTHESIS), the shipped engine over-glues where the spec would allow a break. Documented as a deferral; a future round that adds an EA-W table to SCUD + a per-side LB30 gate closes the gap.
- **LB9's CM/ZWJ fold is state-tracked, not class-rewritten.** Rather than pre-processing the input to fold CM into the preceding class, the walker keeps a `zwj_active` flag and a `cm_fold` short-circuit on each iteration. The distinction matters for LB8a (which needs to see the ZWJ separately from the class it folded into) and for the `Cm | Zwj` guard on the CM/ZWJ classes at the mandatory-break-class boundary. Verified by the ZWJ + ExtPict smoke test (`A ZWJ B` — LB8a suppresses the break at offset 4).
- **Mandatory-break emission is queue-based, not inline.** LB4/LB5's mandatory breaks are emitted AFTER the terminator, not before the next scalar — the walker queues a `pending_mandatory` slot on seeing BK/CR/LF/NL and flushes it on the following iteration (skipping the CR × LF partner per LB5). A `pending_fired` flag short-circuits the pair-table decision on the iteration where the pending fires so we don't emit two breaks at the same offset. Cross-checked against the `A CR LF B` smoke (one mandatory) and `A LF B` (one mandatory) tests.

**Deferred (documented, not implemented).**

- **Dictionary-based line breaking for Thai / Lao / Khmer / Burmese (SA class).** LB1's SA → AL fold is a Phase 5 approximation — the true behaviour is dictionary-based per-script. Deferred to a follow-up SCUD pack extension carrying per-script segmenter dictionaries.
- **CSS-specific `line-break` / `word-break` tailorings beyond CJK strictness.** The three CJK strictness variants (loose / normal / strict) are shipped as the `Strictness` enum + engine setter; CSS `line-break: anywhere` (which allows a break between any two grapheme clusters) and `word-break: break-all` (which allows a break within a word) are layout-level concerns that layer on top of the classification the engine produces — deferred to the future layout-side crate.
- **Bidi-aware line breaking.** UBA (Unicode Bidirectional Algorithm) line breaking wants RTL/LTR run boundaries as additional break opportunities. Not implemented — the shipped engine treats the input as a single directional run.
- **Line-height / width-aware break selection.** Choosing WHICH allowed opportunity to consume for a given rendered width is a layout concern (Knuth-Plass, first-fit, best-fit) — the shipped engine only *enumerates* the opportunities; a downstream layout crate would consume the list.
- **CLDR `line-break-*` locale tailorings beyond CJK strictness.** e.g. Japanese "kinsoku shori" beyond what the strictness tag covers, Chinese "avoid line-start / line-end" punctuation. Deferred.
...
- **Collation locales.** The Phase 2 shipped set of `{en, de}` grows to `{en, de, fr, tr, ru, zh}`. The fr / ru / zh packs shipped in Wave 14 (each ships options + shared ß→ss expansion + Latin-1 supplement rows so the composed engine has uniform pack-hit ratios); the tr pack adds Turkish alphabet options. Locale-specific ordering tailorings — French backwards-secondary, Turkish primary-distinct dotless-ı, Russian case-second, and a Chinese stroke-based scaffold covering ~230 common CJK Ideographs — all landed via the `SECT_PRIMARY_OVERRIDES` section (bytes `PriW`, `u32 count` + sorted `(u32 cp, u32 primary, u32 secondary, u32 tertiary)` records) plus the `backwards_secondary` (byte 2) and `case_second` (byte 3) bits in `SECT_COLLATION_OPTIONS`. Full Chinese stroke dataset and pinyin ordering remain the last collation deferrals (see the red-flag list below).
...
- ~~Russian case-second is a documented deferral.~~ **Landed** (2026-08). `SECT_COLLATION_OPTIONS` grows a `case_second` bit (byte 3, previously reserved — legacy blobs that wrote it as 0 still parse cleanly); the ru collation pack sets it. `CollationEngine::compare_with_case_second` delegates level-1 primary to feruca after a Unicode-aware lowercasing pass (so Ё still sorts between Е and Ж), then breaks primary ties with a per-character case-marker sequence (`0x02` for uppercase, `0x01` for lowercase / non-letter). Precedence when a pack lights multiple option bits: primary-overrides > backwards-secondary > case-second. Result: `"аА" < "Аа"` at secondary strength — the CLDR `ru` `standard` lowercase-before-uppercase rule. +8 unit tests in `stringcheese-icu-collation` + 9 new ru golden assertions replacing the shipped-deferral test; ru golden vector count 38 → 54.
- ~~Chinese stroke-based ordering has no data.~~ **Landed as a scaffold** (2026-08). `stringcheese-zh`'s build.rs authors a starter set of ~230 common CJK Ideographs paired with their stroke count and pushes them onto `SECT_PRIMARY_OVERRIDES` via the existing writer. Weight formula: `1000 + stroke_count * 100 + within_stroke_index`, sorted radical → codepoint within a bucket. Shipped Han sorts by stroke count; un-shipped Han falls through to the primary-override path's codepoint approximation. No `CollationEngine` change needed — the primary-override compare path already services this shape (landed with the Turkish work). Pack size: 4328 bytes. +19 new zh golden assertions replacing the stroke-deferral tests; zh golden vector count 42 → 52.

**Remaining collation deferrals.** Two data-shipping follow-ups still ride on approximations:

- **Full stroke dataset for Chinese.** The ~20 000 CJK glyphs beyond the shipped starter set are a data-only follow-up. Blob budget will grow from ~4 KiB to ~200 KiB when the full CLDR table lands.
- **Chinese pinyin (`zh-u-co-pinyin`) collation.** Requires a ~40 000-entry Han → pinyin table plus tone handling. Still deferred entirely — no scaffold data yet.

### 8.5. Phase 5 progress (2026-08)

**Phase 5 follow-up — CJK word-break dictionaries (2026-08).** Closes the "Dictionary-based CJK word segmentation" deferral flagged below by shipping starter dictionaries plus a forward-maximum-match (FMM) engine hook:

- `stringcheese-scud` — extended with the `SECT_WORD_DICT` section (`b"BkWd"`) plus a `WordDictView` zero-copy accessor and a `BreakSectionBuilder::push_dict_entry` / `word_dict_bytes` writer helper. Wire format: `u32 count`, `u16 max_word_len_bytes`, `u16 reserved`, then a `count * u32` offset index into a packed payload of `(u16 length, [u8; length])` records sorted lexicographically. `longest_prefix_match(input) -> Option<usize>` uses a binary search for the largest entry `<=` input, then walks backwards through entries that share `input`'s first byte until a prefix match is found — this handles the case where `input = "北京很大"` and the dict carries both `北京` and `北京大学` (the largest-`<=` entry `北京大学` is not a prefix; the walk-back finds `北京`). Regression test `word_dict_walks_back_past_non_prefix_neighbour` locks this behaviour.
- `stringcheese-icu-segment` — `BreakEngine::segment_words` now dispatches to a dictionary-driven FMM walker when the loaded pack carries a `WordDictView` and the requested locale's primary subtag is `"ja"` or `"zh"`. The walker splits input into runs of CJK-script scalars (a broad `is_cjk_scalar` classifier covering Han + Hiragana + Katakana + half-width kana + Kanbun symbols) and non-CJK runs — CJK runs consume the dict, non-CJK runs still use the UAX #29 `WordIter`. Unknown-word fallback emits a single scalar as its own word-like segment (a documented red flag; see below). No changes to the shipped WIT surface.
- `stringcheese-ja` — new `break-scud` feature ships `word-dict-ja.scud` (~5.1 KiB, 388 unique entries) authored from a hand-curated public-domain frequency list (pronouns, particles, common verbs in dictionary + polite + past forms, ~100 nouns + kanji compounds like 東京 / 東京大学 / 日本, ~30 adjectives, 14 numeric kanji 一..億, ~50 katakana loanwords). Exposed via `word_dict_data::break_pack()`. 22 FMM golden vectors including 私は学生です → [私, は, 学生, です]; 東京大学に行きます → [東京大学, に, 行きます]; unknown-kanji single-char fallback; mixed CJK/Latin (JavaScript in Japanese text stays a single Latin word).
- `stringcheese-zh` — new `break-scud` feature ships `word-dict-zh.scud` (~4.1 KiB, ~350 unique entries) authored from HSK levels 1-4 vocabulary (pronouns, common verbs, auxiliaries, particles, ~50 nouns + city/country/institution bigrams like 北京 / 上海 / 北京大学 / 中国, ~35 adjectives, 18 numeric hanzi, time bigrams, greetings). Exposed via `word_dict_data::break_pack()`. 22 FMM golden vectors including 我是学生 → [我, 是, 学生]; 北京大学 → [北京大学]; 北京很大 → [北京, 很, 大] (proves the walk-back-past-non-prefix fix); 你好 / 谢谢 / 对不起没关系 as single dict entries.
- CI — extended the existing `wasm-i18n-segment` job's `-p` list to include `stringcheese-ja` and `stringcheese-zh` under `--all-features`, so both `word_segment_ja.rs` and `word_segment_zh.rs` golden-vector suites run on every push. No new job.

**Follow-up deferrals.** Documented, not implemented:

- **Full IPADIC / JMdict integration for Japanese** — 100k+ entries, multi-MB. Ships as a follow-up SCUD pack under the same `break-scud` feature; the shipped ~388-entry starter proves the FMM algorithm shape end-to-end.
- **Full CC-CEDICT integration for Simplified Chinese** — ~130k entries, ~10 MB. Same follow-up shape as the JMdict wave.
- **Traditional Chinese variant (`zh-TW` / `zh-HK`)** — Simplified-only starter dict; Traditional characters fall through the unknown-word single-char path. A Traditional pack is a follow-up wave (shares the same wire format; only the entries differ).
- **Viterbi / CRF-based Japanese segmenter** — MeCab-quality output needs morpheme-level ML models over IPADIC part-of-speech tags. Out of scope for the wasm-first offline pack; the FMM segmenter is O(n) and correct-enough for common vocabulary.
- **Backward maximum match / bidirectional comparison** — a cheap accuracy improvement over pure FMM. Not implemented in this wave.
- **Thai / Khmer / Lao / Burmese syllable segmentation** — different algorithm shape (syllable-formation rules or per-script dictionaries). Still deferred; UAX #14 already carries the `SA → AL` fold as a Phase 5 approximation (see § 8.7).

**Red flags.** A couple worth noting for reviewers:

- **Unknown-word fallback emits single scalars, not larger runs.** When the dict has no match for the character at the current position, the FMM walker emits exactly one scalar (still marked `is_word_like`) and advances. This is the standard FMM contract but it means an unknown compound word like 首都 (capital) splits into 首 and 都 individually rather than clustering as a single Other-class run. Fine for a starter dict; the full dict wave will cover 首都 and reduce the single-char cases dramatically.
- **Locale gating is primary-subtag only.** `locale_wants_cjk_dict` splits on `-` / `_` and matches `"ja"` or `"zh"` on the primary subtag. `"ja-JP"`, `"zh-Hans-CN"`, `"zh-CN"` all engage the dict; `"en"` does not. `"cmn"` / `"yue"` (ISO 639-3 macrolanguage subtags for Mandarin / Cantonese) do NOT engage the dict — those callers would need to pass `"zh"` explicitly. A more permissive matcher is a small follow-up.
...
- **Dictionary-based CJK word segmentation.** **Partially landed** in the 8.5 follow-up above: FMM segmenter shipped in `stringcheese-icu-segment` with `~388-entry` (ja) and `~350-entry` (zh) starter dictionaries in the language packs. Full IPADIC / JMdict / CC-CEDICT integration and MeCab-quality Viterbi/CRF decoding remain follow-ups.
