...
**Locales covered vs deferred.** Phase 3's rule table lets any locale whose CLDR rules happen to be a subset of the encoded predicates ship without further work. Wave 3 above brings the shipped set to **ten** locales: `en`, `de`, `fr`, `es`, `pt`, `ru`, `pl`, `ar`, `zh`, `ja`. Italian remains algorithm-supported (the `EsItPtCardinalMany` and `ItOrdinalMany` opcodes plus `italian_cardinals` / `italian_ordinals` builder helpers ship in this wave) but does not yet have a shipped SCUD pack because `stringcheese-it` does not exist in the workspace — creating that language-pack crate (Snowball Italian stemmer, stopword list, phonetic encoder, `Language` trait impl) is a separate follow-up. Once `stringcheese-it` lands, wiring plural-it / number-it becomes a data-only extension using the same builder helpers. The full ~200-locale CLDR plural table remains a Phase-3-scope deferral: hand-encoding the remaining predicates (locale-specific reorderings, additional operand combinations) is a follow-up wave.
...
- Full ~200-locale CLDR plural rules (Phase 3 targets ~13 covered predicates across the shipped `en`, `de`, `fr`, `es`, `pt`, `ru`, `pl`, `ar`, `zh`, `ja` packs).
- `stringcheese-it` language pack — the Latin-adjacent trio's third member. Algorithm-supported (`italian_cardinals` and `italian_ordinals` builder helpers plus the `EsItPtCardinalMany` / `ItOrdinalMany` opcodes ship in Wave 3), but the crate itself does not exist in the workspace. Wiring plural-it / number-it becomes a data-only extension once the base pack (Snowball Italian stemmer, stopwords, phonetic encoder, `Language` trait impl) is created.
