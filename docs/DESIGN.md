...
| `stringcheese-am` | Amharic pack (**first Ge'ez-script pack** and **first Ethiopian / Eritrean language pack** in StringCheese; Semitic sibling of Arabic and Hebrew, but written in the Ge'ez / Ethiopic abugida rather than either Semitic abjad): ~55-word Amharic-in-Ge'ez stopword list covering personal / demonstrative / interrogative pronouns, prepositions, conjunctions, negation and affirmation particles, and high-frequency copula surface forms; rule-based Amharic suffix stripper — **there is no canonical Snowball Amharic algorithm** — the module ships a deliberately conservative iterate-to-convergence longest-match-wins subset covering the definite article `-ው` (m.) / `-ዋ` (f.), plural `-ኦች`, possessive suffixes (`-ዬ` my, `-ህ` your-m, `-ሽ` your-f, `-ው` his, `-ዋ` her, `-ችን` our, `-ችሁ` your-pl, `-ችው` their), and object suffixes (`-ኝ` me, `-ህ` you-m, `-ሽ` you-f, `-ው` him, `-ት` her, `-ን` us, `-ችሁ` you-pl, `-ኣቸው` them), all matched on `Vec<char>` with a 2-scalar min-stem guard; Ge'ez-aware tokenizer that treats the main Ge'ez block U+1200..=U+137F (minus the Ge'ez punctuation range U+1361..=U+1368) as word-internal along with the supplement U+1380..=U+139F and the extended U+2D80..=U+2DDF blocks, with `፡` (U+1361 wordspace — the traditional Ge'ez word separator), `።` (U+1362 full stop — the sentence terminator), and `፣ ፤ ፥ ፦ ፧ ፨` (comma / semicolon / colon / preface colon / question mark / paragraph separator, U+1363..=U+1368) as explicit separators; dedicated `stringcheese_am::geez` module ships the syllable decompose / compose math — Ge'ez is an **abugida / syllabary** where each scalar represents a **consonant + vowel** combination (unlike Devanagari or Bengali where consonant + matra is two scalars) organized as **48 rows × 8 columns** in the main U+1200..=U+137F block (each row is a consonant family, each column is one of the 7 canonical vowel orders ə/ካዕብ-u/ሣልስ-i/ራብዕ-a/ሓምስ-e/ሳድስ-ɨ/ሳብዕ-o plus an eighth labialized column), so `decompose(scalar) → (family_head, order)` and `compose(family_head, order) → scalar` are pure arithmetic on the offset from U+1200; two-stage BGN/PCGN-style Latin → PHONEX-Amharic phonetic hookup — the [`AmharicBgnPcgn`] transliteration decomposes each syllable via [`geez::decompose`] and looks up the consonant family's ASCII romanization plus the vowel order's suffix (48-family table with linguistic-category groupings: pharyngeal `ḥ→H`, uvular `ḫ→H`, emphatic `ṭ→T ṣ→s p̣→P č̣→C`, sibilant `š→x š/ś→s ž→Z`, palatal `ñ→N`, glottal `'→'`, ayn `ʿ→\``, retroflex `ḍ→D`, and labialized rows fold to their base consonant), and the ASCII output then feeds a Soundex-shape 4-character reduction with the standard consonant classes and vowel-reset (`A/E/I/O/U/Y/H` reset); adapter name `"phonex-am"`, matching the shape of the other Latin-alphabet packs' phonetic hookups. Every Ge'ez main-block scalar is **3 bytes in UTF-8** (U+1200..=U+137F falls in UTF-8's 3-byte range) — all suffix / tokenizer / stemmer arithmetic runs on `Vec<char>` for the same reason as the Bengali and Devanagari packs. Tigrinya (`stringcheese-ti`) and Tigre (`stringcheese-tig`) sibling packs (both share the Ge'ez script but with distinct function-word inventories and morphology), liturgical Ge'ez (`stringcheese-gez`), full Semitic root-and-pattern morphological analysis (needs `HornMorpho` / `AmMorpho` scale template lexicon), verb morphology (subject / object clitics, tense-aspect-mood markers, template-driven stem alternations), ISO 9985 alternate romanization with diacritics, and the Ge'ez supplement + extended blocks' non-8-column syllable layout deferred. Self-registers into `stringcheese-lang::registry` as `"am"`. |
...
| `stringcheese-pa` | Punjabi (Eastern, Gurmukhi script) pack (**first Gurmukhi-script pack** and **fifth Brahmic-script pack** after `stringcheese-hi` (Devanagari), `stringcheese-bn` (Bengali), `stringcheese-ta` (Tamil), and `stringcheese-ml` (Malayalam, when it lands); Gurmukhi script U+0A00..=U+0A7F is a Brahmic-family abugida sibling and every Gurmukhi letter is likewise **3 bytes in UTF-8** — all suffix / tokenizer / stemmer arithmetic runs on `Vec<char>` for the same reason): **Punjabi is a tonal language** — the historical Sanskrit-inherited voiced-aspirate letters `ਘ`/`ਝ`/`ਢ`/`ਧ`/`ਭ` have lost their voicing and aspiration in modern Punjabi and now encode a *tone contour* on the adjacent vowel (low tone at syllable start, high tone at syllable end), and the phone is now the same as the corresponding voiceless-unaspirated stop; ~55-word Gurmukhi stopword list covering personal / demonstrative / interrogative pronouns, postpositions, conjunctions, particles, high-frequency forms of the copula *ਹੋਣਾ* ("to be"), and common adverbs; light Punjabi suffix-stripping stemmer — **there is no canonical Snowball Punjabi algorithm** (Snowball does not list Punjabi; the community references are Kumar & Josan 2010 and Gupta 2013) — the module ships a deliberately conservative rule-based subset covering case markers (`-ੇ` oblique sg, `-ੀ` fem sg), plural markers (`-ਾਂ`, `-ਆਂ`, `-ੀਆਂ`, `-ਿਆਂ`), imperfective participles (`-ਦਾ`/`-ਦੀ`/`-ਦੇ`), and perfective/aorist endings (`-ਿਆ` 3sg-m, `-ੀ` 3sg-f, `-ੇ` 3pl-m, `-ੀਆਂ` 3pl-f), all matched on `Vec<char>` with a 2-scalar min-stem guard (tippi `ੰ` U+0A70, bindi `ਂ` U+0A02, addak `ੱ` U+0A71 never stripped alone); Gurmukhi-aware tokenizer that treats the full Gurmukhi block U+0A00..=U+0A7F as word-internal (letters plus dependent vowel signs / matras / virama / tippi / bindi / addak / nukta) with the Devanagari-inherited danda (`।` U+0964) and double danda (`॥` U+0965) as separators (they sit outside the Gurmukhi block); two-stage ISO 15919 → tone-collapsed PHONEX-Punjabi phonetic hookup — the [`PunjabiIso15919`] transliteration honors the **explicit-schwa** convention (a base consonant with no following matra or virama emits letter + `a`; virama `੍` U+0A4D suppresses; matras override), handles **addak-driven gemination** (`ਪੱਕਾ` → "pakkā" — the addak between p+k doubles the k) and **tippi/bindi nasalization** attached to the preceding vowel (tippi → `ṁ`, bindi → `m̐`), covers the 33 base consonants with retroflex under-dots (`ṭ ṭh ḍ ḍh ṇ`), sibilant diacritics (`ś`), the native retroflex flap `ੜ` → `ṛ`, and the Perso-Arabic nukta letters `ਖ਼ ਗ਼ ਜ਼ ਫ਼` (mapped to `x ġ z f`) both in precomposed and decomposed forms; the ISO output then feeds a **Punjabi-specific tone-collapse pre-pass** that folds the historical voiced-aspirate digrams to their voiceless-unaspirated counterparts (`gh → k`, `jh → c`, `ḍh → ṭ`, `dh → t`, `bh → p`) so that tone-marked and unmarked spellings of the same word (e.g. `ਘਰ` and `ਕਰ`) share a phonex key; the collapsed output finally feeds a standard Soundex-shape 4-character reduction (`ā → A`, `ṭ → T`, `ś → S`, `ġ → G`, `ṛ → R`, `ṁ → M`, etc., then class-code push with vowel-reset); adapter name `"phonex-pa"`. Shahmukhi (Western Punjabi, Perso-Arabic script, BCP-47 `pa-Arab`) sibling pack (`stringcheese-pa-arab`), regional dialect stopwords (Doabi / Malwai / Puadhi / Pothohari), explicit tone-marking transliteration, ITRANS / HK / SLP1 romanization adapters, and full Snowball Punjabi (if one ever appears) deferred. Self-registers into `stringcheese-lang::registry` as `"pa"`. |
