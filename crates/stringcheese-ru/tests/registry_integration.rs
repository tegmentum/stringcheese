//! Verifies that depending on `stringcheese-ru` is enough to make the
//! Russian pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-ru/src/lib.rs` is the
