//! Verifies that depending on `stringcheese-ko` is enough to make the
//! Korean pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-ko/src/lib.rs` is the
...
// Force the `stringcheese_ko` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_KO: &stringcheese_ko::Korean = &stringcheese_ko::KOREAN;

#[test]
fn korean_pack_is_registered() {
    let lang = registry::language("ko").expect("Korean pack must be registered");
    assert_eq!(lang.code(), "ko");
    assert_eq!(lang.name(), "Korean");
}

#[test]
fn korean_pack_registration_is_case_insensitive() {
    for probe in ["KO", "Ko", "kO"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Korean",
        );
    }
}

#[test]
fn korean_pack_functions_through_registry() {
    let lang = registry::language("ko").expect("Korean pack must be registered");
    assert!(lang.is_stopword("그리고"));
    // Coarse particle stripping still fires through the trait object.
    assert_eq!(lang.stem("학교에서"), "학교");
}
