//! Verifies that depending on `stringcheese-sk` is enough to make the
//! Slovak pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-sk/src/lib.rs` is the
...
// Force the `stringcheese_sk` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_SK: &stringcheese_sk::Slovak = &stringcheese_sk::SLOVAK;

#[test]
fn slovak_pack_is_registered() {
    let lang = registry::language("sk").expect("Slovak pack must be registered");
    assert_eq!(lang.code(), "sk");
    assert_eq!(lang.name(), "Slovak");
}

#[test]
fn slovak_pack_registration_is_case_insensitive() {
    for probe in ["SK", "Sk", "sK"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Slovak"
        );
    }
}

#[test]
fn slovak_pack_functions_through_registry() {
    let lang = registry::language("sk").expect("Slovak pack must be registered");
    assert!(lang.is_stopword("a"));
    assert_eq!(lang.stem("pekný"), "pekn");
}
