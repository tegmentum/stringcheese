//! Verifies that depending on `stringcheese-fi` is enough to make the
//! Finnish pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-fi/src/lib.rs` is the
...
// Force the `stringcheese_fi` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_FI: &stringcheese_fi::Finnish = &stringcheese_fi::FINNISH;

#[test]
fn finnish_pack_is_registered() {
    let lang = registry::language("fi").expect("Finnish pack must be registered");
    assert_eq!(lang.code(), "fi");
    assert_eq!(lang.name(), "Finnish");
}

#[test]
fn finnish_pack_registration_is_case_insensitive() {
    for probe in ["FI", "Fi", "fI"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Finnish"
        );
    }
}

#[test]
fn finnish_pack_functions_through_registry() {
    let lang = registry::language("fi").expect("Finnish pack must be registered");
    assert!(lang.is_stopword("ja"));
    assert_eq!(lang.stem("talossani"), "talo");
}
