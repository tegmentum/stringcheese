//! Property tests for the Norwegian (Bokmål) language pack.
//!
...
proptest! {
    ...
    /// every Snowball Norwegian rule is either a delete-suffix or a
    /// replace-with-equal-or-shorter fixed string (`erte → er` and
    /// `ert → er` shorten by 2 and 1 respectively; every other rule
    /// shortens by at least the suffix's length).
    #[test]
    fn snowball_stem_char_count_is_no_longer_than_input(w in norwegian_word()) {
        let out = NorwegianSnowball.stem(&w).into_owned();
        prop_assert!(
    ...
    fn is_stopword_case_invariant_ascii(w in mixed_case_word()) {
        let hit_lower = NORWEGIAN.is_stopword(&w.to_ascii_lowercase());
        let hit_upper = NORWEGIAN.is_stopword(&w.to_ascii_uppercase());
    ...
    fn every_stopword_is_recognized(i in 0usize..STOPWORDS.len()) {
        let w = STOPWORDS[i];
        prop_assert!(NORWEGIAN.is_stopword(w));
        if w.is_ascii() {
            prop_assert!(NORWEGIAN.is_stopword(&w.to_ascii_uppercase()));
        }
    }

    /// The Norwegian phonetic encoder is total on ASCII-alphabetic
    ...
    fn phonex_is_total_on_ascii_alphabetic_input(w in ascii_word()) {
        ...
        let out = NorwegianPhonex.encode(&w);
        prop_assert!(
            out.is_some(),
            "NorwegianPhonex returned None for {:?}",
            w
        );
    }
    ...
    fn phonex_key_is_always_four_chars(w in norwegian_word()) {
        if let Some(k) = NorwegianPhonex.encode(&w) {
    ...
    fn phonex_is_case_invariant_ascii(w in mixed_case_word()) {
        let a = NorwegianPhonex.encode(&w.to_ascii_lowercase());
        let b = NorwegianPhonex.encode(&w.to_ascii_uppercase());
        prop_assert_eq!(a, b);
    }
    ...
    fn tokenizer_empty_input_yields_zero_tokens(_dummy in 0u8..1) {
        let toks: Vec<&str> = NorwegianTokenizer::new().tokenize("").collect();
    ...
    fn tokenizer_never_invents_characters(text in norwegian_text()) {
        let toks: Vec<&str> = NorwegianTokenizer::new().tokenize(&text).collect();
        for t in &toks {
            for c in t.chars() {
                prop_assert!(
                    text.contains(c),
    ...
    fn tokenizer_never_yields_empty_tokens(text in norwegian_text()) {
        for t in NorwegianTokenizer::new().tokenize(&text) {
