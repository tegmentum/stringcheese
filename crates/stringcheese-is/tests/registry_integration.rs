//! Verifies that depending on `stringcheese-is` is enough to make the
//! Icelandic pack visible through [`stringcheese_lang::registry`].
//! The `register_language!` macro in `stringcheese-is/src/lib.rs` is
...
// Force the `stringcheese_is` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_IS: &stringcheese_is::Icelandic = &stringcheese_is::ICELANDIC;

#[test]
fn icelandic_pack_is_registered() {
    let lang = registry::language("is").expect("Icelandic pack must be registered");
    assert_eq!(lang.code(), "is");
    assert_eq!(lang.name(), "Icelandic");
}

#[test]
fn icelandic_pack_registration_is_case_insensitive() {
    for probe in ["IS", "Is", "iS"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Icelandic"
        );
    }
}

#[test]
fn icelandic_pack_functions_through_registry() {
    let lang = registry::language("is").expect("Icelandic pack must be registered");
    assert!(lang.is_stopword("og"));
    assert_eq!(lang.stem("hesturinn"), "hest");
}
