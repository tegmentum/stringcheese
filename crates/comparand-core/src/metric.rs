...
pub trait DistanceMetric<S: ?Sized> {
    ...
    ///
    /// These claims are exercised by Comparand's property-based test suite;
...
/// observations on a particular corpus. Comparand's property-based test
...
/// theory. `normalized` is a boundedness claim specific to Comparand:
...
///
/// Every algorithm in Comparand belongs to exactly one class. The class is
