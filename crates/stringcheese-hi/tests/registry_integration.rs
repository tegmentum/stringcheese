//! Verifies that depending on `stringcheese-hi` is enough to make
//! the Hindi pack visible through
//! [`stringcheese_lang::registry`]. The `register_language!` macro
//! in `stringcheese-hi/src/lib.rs` is the sole opt-in — if it stops
...
// Force the `stringcheese_hi` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_HI: &stringcheese_hi::Hindi = &stringcheese_hi::HINDI;

#[test]
fn hindi_pack_is_registered() {
    let lang = registry::language("hi").expect("Hindi pack must be registered");
    assert_eq!(lang.code(), "hi");
    assert_eq!(lang.name(), "Hindi");
}

#[test]
fn hindi_pack_registration_is_case_insensitive() {
    for probe in ["HI", "Hi", "hI"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Hindi"
        );
    }
}

#[test]
fn hindi_pack_functions_through_registry() {
    let lang = registry::language("hi").expect("Hindi pack must be registered");
    assert!(lang.is_stopword("और"));
    assert_eq!(lang.stem("लड़का"), "लड़क");
}
