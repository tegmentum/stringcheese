//! Canonical Levenshtein golden cases wired to the `comparand-corpus` schema.
//!
...
pub const GOLDEN_CHARS: &[CharsCase] = &[
    GoldenCase {
        ...
        expected: Distance::new(1),
        source: GoldenSource::IndependentlyDerived,
        notes: "Diacritic difference at char-slice granularity is one substitution; at byte granularity the answer would be two (é is two bytes in UTF-8), which is exactly the sort of silent representation choice Comparand refuses to make.",
