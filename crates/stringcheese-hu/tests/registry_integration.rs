//! Verifies that depending on `stringcheese-hu` is enough to make the
//! Hungarian pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-hu/src/lib.rs` is the
...
// Force the `stringcheese_hu` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_HU: &stringcheese_hu::Hungarian = &stringcheese_hu::HUNGARIAN;

#[test]
fn hungarian_pack_is_registered() {
    let lang = registry::language("hu").expect("Hungarian pack must be registered");
    assert_eq!(lang.code(), "hu");
    assert_eq!(lang.name(), "Hungarian");
}

#[test]
fn hungarian_pack_registration_is_case_insensitive() {
    for probe in ["HU", "Hu", "hU"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Hungarian"
        );
    }
}

#[test]
fn hungarian_pack_functions_through_registry() {
    let lang = registry::language("hu").expect("Hungarian pack must be registered");
    assert!(lang.is_stopword("és"));
    assert_eq!(lang.stem("házban"), "ház");
}
