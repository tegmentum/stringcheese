...
//!
//! [`CharacterGrams`]: stringcheese_ngram::CharacterGrams
//! [`GramSet`]: stringcheese_ngram::GramSet
//! [`GramSet::from_generator`]: stringcheese_ngram::GramSet::from_generator
//! [`GramMultiSet`]: stringcheese_ngram::GramMultiSet
//! [`GramVector`]: stringcheese_ngram::GramVector
//! [`Vec`]: alloc::vec::Vec
...
/// [`stringcheese_ngram::CharacterGrams`] generator through every case. The
...
pub enum SetSimShape {
    ...
    /// gram, deduplicated into a [`stringcheese_ngram::GramSet`].
    Set,
    /// Same character list, but preserved as a multiset with each
    /// character counted separately, stored in a
    /// [`stringcheese_ngram::GramMultiSet<char>`].
    MultiSet,
    /// Same character list, materialized as a
    /// [`stringcheese_ngram::GramVector<char>`] whose per-gram weight is the
    /// count of that character in the list.
    Vector,
    /// The character list is treated as a raw input string, passed through
    /// a character-3-gram generator with [`stringcheese_ngram::PaddingPolicy::None`],
    /// and consumed as a [`stringcheese_ngram::GramSet<Vec<char>>`].
    Chars3Gram,
}
...
mod tests {
    ...
    use stringcheese_ngram::{CharacterGrams, GramMultiSet, GramSet, GramVector, PaddingPolicy};
