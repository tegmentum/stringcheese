//! Set-similarity metrics for the StringCheese toolkit.
//!
//! This crate is StringCheese's fifth vertical slice, and the first that
//! *consumes* another algorithm crate's output: every algorithm here takes
//! one of `stringcheese-ngram`'s representation types ([`GramSet`],
//! [`GramMultiSet`], or [`GramVector`]) rather than a raw slice. Callers
//! commit to a representation up-front — a strict application of StringCheese's
...
//! `sim(∅, ∅)`. StringCheese adopts the identity-of-indiscernibles convention
...
//! matches the convention Jaro uses ([`stringcheese_jaro::Jaro`] returns
...
//! that IS a metric.
//!
//! [`class`]: stringcheese_core::SimilarityMetric::class
//! [ng]: https://github.com/tegmentum/stringcheese/blob/main/docs/design/ngram-and-fingerprinting.md
//! [ts]: https://github.com/tegmentum/stringcheese/blob/main/docs/design/type-system.md
//! [gs]: stringcheese_ngram::GramSet::from_generator
//!
//! # `no_std`
//!
...
//! [`stringcheese_jaro::Jaro`]: https://docs.rs/stringcheese-jaro
//!
//! # References
//!
