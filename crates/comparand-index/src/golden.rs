...
//! `comparand-levenshtein`'s golden set). The q-gram tests use hand-built
//! `Vec<u8>` grams so the expected candidate lists can be enumerated by
//! eye without pulling in the n-gram crate.
//!
//! [`GoldenSource::IndependentlyDerived`]: comparand_corpus::GoldenSource::IndependentlyDerived
//! [`Levenshtein`]: comparand_levenshtein::Levenshtein
