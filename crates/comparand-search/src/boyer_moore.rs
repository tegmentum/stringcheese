//! Boyer-Moore substring search over `&[u8]` — bad-character heuristic only.
//!
//! # Algorithm
//!
...
//! pattern index `j` against haystack byte `b`, the shift is
//! `max(1, j - last_occurrence[b])`.
//!
//! # Heuristic scope
//!
//! This is the *bad-character-only* variant. The good-suffix heuristic —
//! which contributes independent shift information from the *matched*
//! suffix rather than the mismatching character — is not implemented here.
//! Adding it in a future version will produce a distinct descriptor
//! variant (`"full-with-good-suffix"`) so no golden case is silently
//! upgraded.
//!
...
//! in practice on natural text with a large alphabet. The crate-internal
//! differential property suite pins its output against Rabin-Karp and KMP
//! over random inputs.
//!
//! # Descriptor
//!
//! The variant slug is `"bad-character-only"`. Golden test cases reference
//! this variant so that the future full-Boyer-Moore variant cannot be
//! silently validated against these cases.
