...
// `rules/en.toml`. Exposes:
//   - static STOPWORDS: &[&str]
//   - pub static CAPABILITIES: stringcheese_lang::LanguageCapabilities
//
// The `include!` sits at the crate root so the generated `pub static`
// binding is visible as `stringcheese_en::CAPABILITIES` for callers
// walking the data-driven surface directly. `stopwords::STOPWORDS`
// re-exports it under the historical module path.
mod generated {
...
mod pack {
    ...
    impl English {
        ...
        ///
        /// This is the default; the method exists so a caller who
        /// composed a `with_cldr_collator` pack (available when the
        /// `icu-collator` feature is enabled) can undo that choice.
        #[must_use]
