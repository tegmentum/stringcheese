//! Verifies that depending on `stringcheese-en` is enough to make the
//! English pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-en/src/lib.rs` is the
...
stringcheese_lang::pack_registry_smoke_test! {
    pack: stringcheese_en::ENGLISH,
    pack_ty: stringcheese_en::English,
    code: "en",
    name: "English",
    smoke: |lang| {
        // Delegate through the trait — proves the registered object is
        // a real Language, not just a shell.
