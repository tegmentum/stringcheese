...
//! six-locale roster from `docs/design/wit-i18n.md` § 8.2:
//!
//! * [`stringcheese-en`] — reference English collation options.
//! * [`stringcheese-de`] — DIN 5007-2 (phonebook) tailoring
//!   (`ä → ae`, `ö → oe`, `ü → ue`, `ß → ss`).
//! * [`stringcheese-fr`] — French collation options (backwards-
//!   secondary is a documented Phase 6 deferral).
//! * [`stringcheese-tr`] — Turkish collation options (primary-
//!   distinct dotless-ı ordering is a documented Phase 6
//!   deferral).
//! * [`stringcheese-ru`] — Russian Cyrillic options (case-second
//!   ordering is a documented Phase 6 deferral).
//! * [`stringcheese-zh`] — Chinese collation options (pinyin /
//!   stroke ordering is a documented Phase 6 deferral).
//!
//! The design commits to a future world where a caller composes a
//! bare `stringcheese-icu-collation` component with per-locale pack
...
//! case-component and tokenizer-component precedents.
//!
//! The bundled packs are the same byte constants each language crate
//! exposes (`collation_data::COLLATION_<LANG>_SCUD`) — reused
//! verbatim, never duplicated, so a fix to any pack lands in one
//! place.
//!
...
/// Build a fresh `CollationEngine` backed by the bundled six-locale
...
pub fn reference_engine() -> CollationEngine<'static> {
    CollationEngine::new(reference_packs())
}
...
/// A single reusable pack list — the `CollationPack<'static>` values
/// this crate exports so downstream native tests can build their own
/// `CollationEngine` variants without redoing the bundled-pack parse.
///
/// # Panics
///
...
pub fn reference_packs() -> alloc::vec::Vec<CollationPack<'static>> {
    ...
    let fr = stringcheese_fr::collation_data::collation_pack()
        .expect("bundled collation-fr.scud must parse (packaging bug otherwise)");
    let tr = stringcheese_tr::collation_data::collation_pack()
        .expect("bundled collation-tr.scud must parse (packaging bug otherwise)");
    let ru = stringcheese_ru::collation_data::collation_pack()
        .expect("bundled collation-ru.scud must parse (packaging bug otherwise)");
    let zh = stringcheese_zh::collation_data::collation_pack()
        .expect("bundled collation-zh.scud must parse (packaging bug otherwise)");
...
mod tests {
    ...
    #[test]
    fn reference_engine_exercises_new_locale_packs() {
        // Phase 6 six-locale roster: exercise fr, tr, ru, zh via
        // compare() calls to confirm their pack lookup path works.
        let engine = reference_engine();
        // French: standard ASCII primary ordering.
        assert_eq!(
            engine.compare("apple", "banana", "fr", CollationStrength::Primary),
            Ordering::Less,
        );
        // Turkish: standard ASCII primary ordering (the primary-
        // distinct dotless-ı ordering is a Phase 6 deferral).
        assert_eq!(
            engine.compare("a", "b", "tr", CollationStrength::Primary),
            Ordering::Less,
        );
        // Russian: Cyrillic primary ordering (а < б).
        assert_eq!(
            engine.compare("а", "б", "ru", CollationStrength::Primary),
            Ordering::Less,
        );
        // Chinese: Han primary ordering (a < b in mixed ASCII input).
        assert_eq!(
            engine.compare("apple", "banana", "zh", CollationStrength::Primary),
            Ordering::Less,
        );
    }
