//! Verifies that depending on `stringcheese-et` is enough to make the
//! Estonian pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-et/src/lib.rs` is the
...
// Force the `stringcheese_et` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_ET: &stringcheese_et::Estonian = &stringcheese_et::ESTONIAN;

#[test]
fn estonian_pack_is_registered() {
    let lang = registry::language("et").expect("Estonian pack must be registered");
    assert_eq!(lang.code(), "et");
    assert_eq!(lang.name(), "Estonian");
}

#[test]
fn estonian_pack_registration_is_case_insensitive() {
    for probe in ["ET", "Et", "eT"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Estonian"
        );
    }
}

#[test]
fn estonian_pack_functions_through_registry() {
    let lang = registry::language("et").expect("Estonian pack must be registered");
    assert!(lang.is_stopword("ja"));
    assert_eq!(lang.stem("majas"), "maja");
}
