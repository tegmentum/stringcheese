//! Canonical LCS golden cases wired to the `comparand-corpus` schema.
//!
//! LCS is packaged in this crate as two closely related algorithms with
//! distinct descriptors: [`Lcs`] reports the LCS *length* as a
//! [`Score<u32>`](comparand_core::Score); [`LcsDistance`] reports the
//! derived edit metric as a [`Distance<u32>`](comparand_core::Distance).
...
//! into the crate's test binaries — `comparand-corpus` is declared as a
//! dev-dependency for exactly that reason.
//!
//! [`AlgorithmFamily`]: comparand_core::AlgorithmFamily
//! [`VariantId`]: comparand_core::VariantId

use comparand_core::{Distance, Score};
use comparand_corpus::{GoldenCase, GoldenSource};

use crate::algorithm::{Lcs, LcsDistance};
...
pub const LENGTH_CHARS_CASES: &[LengthCharsCase] = &[GoldenCase {
    ...
    source: GoldenSource::IndependentlyDerived,
    notes: "Diacritic difference at char-slice granularity: the LCS is \"caf\" of length 3. At byte granularity the answer would differ (é is two bytes in UTF-8), which is exactly the sort of silent representation choice Comparand refuses to make.",
