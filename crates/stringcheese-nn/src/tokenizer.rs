//! [`NynorskTokenizer`] — a thin wrapper around [`SimpleTokenizer`].
//!
//! Nynorsk, like Bokmål and German and Dutch, does not use French-style
//! clitic elision (`l'`/`d'`/`qu'`) and does not need attached-particle
...
//!
//! There is therefore no reason for the Nynorsk pack to ship its own
//! tokenizer implementation — this module exposes [`NynorskTokenizer`]
//! as a **transparent wrapper** around [`SimpleTokenizer`] so the
//! pack's public surface still names a Nynorsk-specific tokenizer
//! type (a courtesy for callers who match the language-pack pattern)
//! without duplicating any splitting logic.
//!
//! # Non-goals
//!
//! - **Compound-noun splitting.** Nynorsk productively compounds nouns
//!   (`fotball + lag → fotballag`). The tokenizer emits compounds as
//!   single tokens; splitting them requires a compound-noun dictionary
//!   and is out of scope for a starter pack.
//! - **Hyphenated compounds.** Nynorsk sometimes hyphenates for
...
/// The Nynorsk tokenizer.
///
/// A zero-sized value; a transparent wrapper around
/// [`stringcheese_lang::SimpleTokenizer`]. See the
/// [module-level docs](self) for why Nynorsk does not need a bespoke
/// splitter.
///
/// # Example
///
/// ```
/// use stringcheese_nn::NynorskTokenizer;
///
/// let toks: Vec<&str> = NynorskTokenizer::new()
///     .tokenize("Katten søv på matta.")
///     .collect();
/// assert_eq!(toks, ["Katten", "søv", "på", "matta"]);
/// ```
#[derive(Copy, Clone, Debug, Default, PartialEq, Eq, Hash)]
pub struct NynorskTokenizer;

impl NynorskTokenizer {
    /// Constructs a new [`NynorskTokenizer`]. Zero-sized; free to
    /// call.
    #[inline]
    #[must_use]
...
mod tests {
    ...
    fn collect(input: &str) -> Vec<&str> {
        NynorskTokenizer::new().tokenize(input).collect()
    }
    ...
    #[test]
    fn splits_a_nynorsk_sentence() {
        assert_eq!(collect("Katten søv."), ["Katten", "søv"]);
    }
    ...
    fn compound_nouns_stay_intact() {
        // Nynorsk compound nouns are written as single orthographic
        // words.
        assert_eq!(
    ...
    fn hyphen_splits_tokens() {
        // Nynorsk sometimes hyphenates (e-post "email"). The simple
