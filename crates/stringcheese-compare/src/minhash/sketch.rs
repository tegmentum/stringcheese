...
/// value under the [`crate::minhash::hash`] construction is `~2^-64` per
...
/// descriptor (mirroring `stringcheese-index`'s rule that BK-trees and
...
impl MinHashSketch {
    ...
    /// participates in. Present for parity with the other StringCheese
    ...
    /// matching `stringcheese-set-similarity`'s crate-wide convention.
    ///
    /// # Panics
    ///
