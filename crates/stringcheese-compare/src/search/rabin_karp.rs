...
//!
//! `stringcheese-cdc` also needs a polynomial rolling hash for content-defined
//! chunking. Both crates are landing in parallel; the two rolling-hash
//! implementations should be consolidated into a shared
//! `stringcheese-fingerprint` crate once both are settled. For now the
//! implementation is inlined here to keep the crate self-contained.
//!
//! TODO(stringcheese-cdc): consolidate rolling-hash into `stringcheese-fingerprint`.
//!
//! # Descriptor
//!
