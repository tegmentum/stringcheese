//! Verifies that depending on `stringcheese-da` is enough to make the
//! Danish pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-da/src/lib.rs` is the
...
// Force the `stringcheese_da` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_DA: &stringcheese_da::Danish = &stringcheese_da::DANISH;

#[test]
fn danish_pack_is_registered() {
    let lang = registry::language("da").expect("Danish pack must be registered");
    assert_eq!(lang.code(), "da");
    assert_eq!(lang.name(), "Danish");
}

#[test]
fn danish_pack_registration_is_case_insensitive() {
    for probe in ["DA", "Da", "dA"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Danish"
        );
    }
}

#[test]
fn danish_pack_functions_through_registry() {
    let lang = registry::language("da").expect("Danish pack must be registered");
    assert!(lang.is_stopword("og"));
    assert_eq!(lang.stem("sæden"), "sæd");
}
