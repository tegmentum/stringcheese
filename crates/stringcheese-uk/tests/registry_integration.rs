...
// Force the `stringcheese_uk` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_UK: &stringcheese_uk::Ukrainian = &stringcheese_uk::UKRAINIAN;

#[test]
fn ukrainian_pack_is_registered() {
    let lang = registry::language("uk").expect("Ukrainian pack must be registered");
    assert_eq!(lang.code(), "uk");
    assert_eq!(lang.name(), "Ukrainian");
}

#[test]
fn ukrainian_pack_registration_is_case_insensitive() {
    for probe in ["UK", "Uk", "uK"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Ukrainian"
        );
    }
}

#[test]
fn ukrainian_pack_functions_through_registry() {
    let lang = registry::language("uk").expect("Ukrainian pack must be registered");
