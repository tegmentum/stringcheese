//! Reusable scratch buffer for Levenshtein edit-distance kernels.
//!
//! Every dynamic-programming kernel in this crate stores its rolling row (or
//! band) in a single flat `Vec<u32>`. When a caller performs many comparisons
//! against a fixed query — record linkage, deduplication, top-k queries — that
//! `Vec` is allocated once and grown on demand, rather than being freshly
//! allocated inside every call.
//!
//! The workspace implements [`Workspace`] from `stringcheese-core`, so generic
...
/// Scratch buffer used by the Levenshtein kernels.
///
/// A `LevenshteinWorkspace` holds a single `Vec<u32>`. Its capacity is the
/// only observable state; the buffer's contents are treated as scratch and
/// are overwritten on every kernel call.
///
/// # Sizing
///
/// The rolling-rows kernel needs `min(len_a, len_b) + 1` cells. The banded
/// kernel needs `min(len_a, len_b) + 1` cells as well (the full inner
/// dimension, indexed by column, with cells outside the band held at a
/// sentinel value). The kernels grow the workspace via
/// [`Workspace::ensure_capacity`]; callers do not need to size it themselves.
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let ws = LevenshteinWorkspace::new();
        assert_eq!(ws.capacity(), 0);
    }

    #[test]
    fn with_capacity_preallocates() {
        let ws = LevenshteinWorkspace::with_capacity(32);
    ...
    fn shrink_releases_capacity() {
        let mut ws = LevenshteinWorkspace::with_capacity(128);
        assert!(ws.capacity() >= 128);
        // Force the underlying Vec to have some slack.
        ws.ensure_capacity(4);
        ws.shrink();
        // The Vec is now sized to its length (4 cells from with_capacity's
        // resize). We assert only that shrink did not increase capacity —
        // Vec::shrink_to_fit is allowed to keep some slack.
        assert!(ws.capacity() <= 128);
    }
}
