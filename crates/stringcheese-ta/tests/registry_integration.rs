//! Verifies that depending on `stringcheese-ta` is enough to make
//! the Tamil pack visible through
//! [`stringcheese_lang::registry`]. The `register_language!` macro
//! in `stringcheese-ta/src/lib.rs` is the sole opt-in — if it stops
...
// Force the `stringcheese_ta` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_TA: &stringcheese_ta::Tamil = &stringcheese_ta::TAMIL;

#[test]
fn tamil_pack_is_registered() {
    let lang = registry::language("ta").expect("Tamil pack must be registered");
    assert_eq!(lang.code(), "ta");
    assert_eq!(lang.name(), "Tamil");
}

#[test]
fn tamil_pack_registration_is_case_insensitive() {
    for probe in ["TA", "Ta", "tA"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Tamil"
        );
    }
}

#[test]
fn tamil_pack_functions_through_registry() {
    let lang = registry::language("ta").expect("Tamil pack must be registered");
    assert!(lang.is_stopword("மற்றும்"));
    assert_eq!(lang.stem("மாணவர்கள்"), "மாணவர்");
}
