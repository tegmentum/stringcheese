//! Verifies that depending on `stringcheese-bn` is enough to make the
//! Bengali pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-bn/src/lib.rs` is the
...
stringcheese_lang::pack_registry_smoke_test! {
    pack: stringcheese_bn::BENGALI,
    pack_ty: stringcheese_bn::Bengali,
    code: "bn",
    name: "Bengali",
    smoke: |lang| {
