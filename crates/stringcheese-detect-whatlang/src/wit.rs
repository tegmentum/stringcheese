//! WIT `Guest` implementation for the tier-1 whatlang wrapper.
//!
//! Bridges the crate's native API ([`crate::detect`], [`crate::detect_from`],
//! [`crate::detect_ranked`], [`crate::capabilities`]) to the shared
//! `tegmentum:lang-detect@0.2.0` interface so per-script builds ship as
//! WASM components the host loads on demand.
//!
...
impl Guest for Component {
    ...
    fn detect_mixed(_text: String) -> Option<Vec<SpanDetection>> {
        // Whatlang has no mixed-language segmentation — that's tier 2.
        None
    }
}
