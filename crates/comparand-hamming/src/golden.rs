//! Canonical Hamming golden cases wired to the `comparand-corpus` schema.
//!
...
pub const GOLDEN_CHARS: &[CharsCase] = &[
    GoldenCase {
        ...
        expected: Distance::new(1),
        source: GoldenSource::IndependentlyDerived,
        notes: "Diacritic difference at char-slice granularity is one mismatch. At byte granularity the two inputs would have different lengths (é is two bytes in UTF-8) and the trait method would panic — exactly the sort of silent representation choice Comparand refuses to make.",
...
mod tests {
    use super::*;
    use crate::algorithm::Hamming;
    use comparand_core::{BoundedDistanceMetric, DistanceMetric};
