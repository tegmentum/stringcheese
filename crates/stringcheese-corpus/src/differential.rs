...
//! StringCheese design document commits to for this classification. Every
...
pub struct DifferentialResult {
    ...
    /// output was compared against StringCheese's own. For example
...
///
/// * [`StringCheeseDefect`] and [`ExternalImplementationBug`] point at fixable
///   bugs — in StringCheese or upstream, respectively.
...
/// [`StringCheeseDefect`]: DifferenceClassification::StringCheeseDefect
...
pub enum DifferenceClassification {
    ...
    Agreement,
    /// StringCheese's implementation was wrong; a bug should be filed and
    /// the golden case retained as a permanent regression.
    StringCheeseDefect,
    /// The external implementation was wrong; the disagreement is
    /// evidence for the upstream project, not for StringCheese. Documented
...
mod tests {
    use super::*;
    use stringcheese_core::{AlgorithmFamily, DefinitionSource, DescriptorVersion, VariantId};
    ...
    fn classification_variants_are_distinct() {
        ...
        assert_ne!(
            DifferenceClassification::StringCheeseDefect,
