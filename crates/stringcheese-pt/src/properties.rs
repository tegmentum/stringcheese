//! Property tests for the Portuguese language pack.
//!
...
/// Strategy for a Portuguese-flavoured word (ASCII plus accents / ç).
fn portuguese_word() -> impl Strategy<Value = String> {
    prop::string::string_regex("[a-zA-ZáéíóúâêôãõçÁÉÍÓÚÂÊÔÃÕÇ]{1,20}")
        .expect("static regex is valid")
}

/// Strategy for arbitrary short Portuguese-flavoured text — letters,
/// accents, spaces, punctuation.
fn portuguese_text() -> impl Strategy<Value = String> {
    prop::string::string_regex(r"[a-zA-Záéíóúâêôãõç ,.!?]{0,40}").expect("static regex is valid")
}
...
proptest! {
    ...
    /// every Snowball Portuguese rule is either a delete-suffix or a
    /// replace-with-shorter-fixed-string, plus the postlude which folds
    /// two-char `a~`/`o~` back into single-char `ã`/`õ` (char-count
    /// non-increasing).
    #[test]
    fn snowball_stem_char_count_is_no_longer_than_input(w in portuguese_word()) {
        let out = PortugueseSnowball.stem(&w).into_owned();
        prop_assert!(
    ...
    /// `str::eq_ignore_ascii_case`). Portuguese accented stopwords
    /// (`não`, `há`, `está`, …) require exact match on the accented
    /// form — the default trait implementation does not fold non-ASCII
    /// scalars — and that's documented in [`crate::stopwords`].
    #[test]
    fn is_stopword_case_invariant_ascii(w in mixed_case_word()) {
        let hit_lower = PORTUGUESE.is_stopword(&w.to_ascii_lowercase());
        let hit_upper = PORTUGUESE.is_stopword(&w.to_ascii_uppercase());
    ...
    fn every_stopword_is_recognized(i in 0usize..STOPWORDS.len()) {
        let w = STOPWORDS[i];
        prop_assert!(PORTUGUESE.is_stopword(w));
        if w.is_ascii() {
            prop_assert!(PORTUGUESE.is_stopword(&w.to_ascii_uppercase()));
        }
    }

    /// The Portuguese phonetic encoder is total on ASCII-alphabetic
    ...
    fn phonex_is_total_on_ascii_alphabetic_input(w in ascii_word()) {
        ...
        let out = PortuguesePhonex.encode(&w);
        prop_assert!(
            out.is_some(),
            "PortuguesePhonex returned None for {:?}",
            w
        );
    }
    ...
    /// returns Some.
    #[test]
    fn phonex_key_is_always_four_chars(w in portuguese_word()) {
        if let Some(k) = PortuguesePhonex.encode(&w) {
    ...
    fn phonex_is_case_invariant_ascii(w in mixed_case_word()) {
        let a = PortuguesePhonex.encode(&w.to_ascii_lowercase());
        let b = PortuguesePhonex.encode(&w.to_ascii_uppercase());
        prop_assert_eq!(a, b);
    }
    ...
    fn tokenizer_empty_input_yields_zero_tokens(_dummy in 0u8..1) {
        let toks: Vec<&str> = PortugueseTokenizer::new().tokenize("").collect();
        prop_assert!(toks.is_empty());
    }

    /// The tokenizer never invents characters.
    #[test]
    fn tokenizer_never_invents_characters(text in portuguese_text()) {
        let toks: Vec<&str> = PortugueseTokenizer::new().tokenize(&text).collect();
        for t in &toks {
            for c in t.chars() {
                prop_assert!(
                    text.contains(c),
    ...
    /// No token is empty.
    #[test]
    fn tokenizer_never_yields_empty_tokens(text in portuguese_text()) {
        for t in PortugueseTokenizer::new().tokenize(&text) {
