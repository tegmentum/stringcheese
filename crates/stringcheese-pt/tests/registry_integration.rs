//! Verifies that depending on `stringcheese-pt` is enough to make
//! the Portuguese pack visible through
//! [`stringcheese_lang::registry`]. The `register_language!` macro
//! in `stringcheese-pt/src/lib.rs` is the sole opt-in — if it stops
...
// Force the `stringcheese_pt` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_PT: &stringcheese_pt::Portuguese = &stringcheese_pt::PORTUGUESE;

#[test]
fn portuguese_pack_is_registered() {
    let lang = registry::language("pt").expect("Portuguese pack must be registered");
    assert_eq!(lang.code(), "pt");
    assert_eq!(lang.name(), "Portuguese");
}

#[test]
fn portuguese_pack_registration_is_case_insensitive() {
    for probe in ["PT", "Pt", "pT"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Portuguese"
        );
    }
}

#[test]
fn portuguese_pack_functions_through_registry() {
    let lang = registry::language("pt").expect("Portuguese pack must be registered");
    assert!(lang.is_stopword("o"));
    assert_eq!(lang.stem("falando"), "fal");
}
