//! Verifies that depending on `stringcheese-es` is enough to make
//! the Spanish pack visible through
//! [`stringcheese_lang::registry`]. The `register_language!` macro
//! in `stringcheese-es/src/lib.rs` is the sole opt-in — if it stops
...
// Force the `stringcheese_es` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_ES: &stringcheese_es::Spanish = &stringcheese_es::SPANISH;

#[test]
fn spanish_pack_is_registered() {
    let lang = registry::language("es").expect("Spanish pack must be registered");
    assert_eq!(lang.code(), "es");
    assert_eq!(lang.name(), "Spanish");
}

#[test]
fn spanish_pack_registration_is_case_insensitive() {
    for probe in ["ES", "Es", "eS"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Spanish"
        );
    }
}

#[test]
fn spanish_pack_functions_through_registry() {
    let lang = registry::language("es").expect("Spanish pack must be registered");
    assert!(lang.is_stopword("el"));
    assert_eq!(lang.stem("hablando"), "habl");
}
