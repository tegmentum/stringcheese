...
//! Vietnamese pack visible through [`stringcheese_lang::registry`].
//! The `register_language!` macro in `stringcheese-vi/src/lib.rs` is
...
// Force the `stringcheese_vi` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_VI: &stringcheese_vi::Vietnamese = &stringcheese_vi::VIETNAMESE;

#[test]
fn vietnamese_pack_is_registered() {
    let lang = registry::language("vi").expect("Vietnamese pack must be registered");
    assert_eq!(lang.code(), "vi");
    assert_eq!(lang.name(), "Vietnamese");
}

#[test]
fn vietnamese_pack_registration_is_case_insensitive() {
    for probe in ["VI", "Vi", "vI"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Vietnamese"
        );
    }
}

#[test]
fn vietnamese_pack_functions_through_registry() {
    let lang = registry::language("vi").expect("Vietnamese pack must be registered");
