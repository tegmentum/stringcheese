//! Verifies that depending on `stringcheese-nl` is enough to make the
//! Dutch pack visible through [`stringcheese_lang::registry`]. The
//! `register_language!` macro in `stringcheese-nl/src/lib.rs` is the
...
// Force the `stringcheese_nl` rlib into the test binary's link — a
...
#[allow(dead_code)]
const KEEP_NL: &stringcheese_nl::Dutch = &stringcheese_nl::DUTCH;

#[test]
fn dutch_pack_is_registered() {
    let lang = registry::language("nl").expect("Dutch pack must be registered");
    assert_eq!(lang.code(), "nl");
    assert_eq!(lang.name(), "Dutch");
}

#[test]
fn dutch_pack_registration_is_case_insensitive() {
    for probe in ["NL", "Nl", "nL"] {
        assert!(
            registry::language(probe).is_some(),
            "{probe:?} did not resolve to Dutch"
        );
    }
}

#[test]
fn dutch_pack_functions_through_registry() {
    let lang = registry::language("nl").expect("Dutch pack must be registered");
    assert!(lang.is_stopword("de"));
    assert_eq!(lang.stem("lopen"), "lop");
}
